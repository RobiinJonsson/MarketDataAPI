        ...
        if not database_exists():
            ...
            if click.confirm("Would you like to initialize a new database now?"):
                ...
                if init_database():
                    ...
                    console.print("[dim]Run 'marketdata init --force' to try manual initialization.[/dim]")
                    return False
            else:
                console.print("\n[dim]To initialize manually, run: [bold]marketdata init[/bold][/dim]")
                console.print("[dim]Or set custom database path with: [bold]SQLITE_DB_PATH=/path/to/db[/bold][/dim]")
                return False
        return True
        ...
        try:
            ...
            if "no such table" in str(e).lower():
                ...
                if click.confirm("Would you like to initialize the database tables now?"):
                    if init_database():
                        console.print("[green]✓ Database initialized. Please retry your command.[/green]")
                    else:
                        console.print("[red]✗ Database initialization failed.[/red]")
                return
            else:
...
def get(ctx, isin):
    ...
    try:
        ...
        if not instrument.figi_mappings:
            ...
            console.print("Run '[cyan]marketdata instruments enrich {isin}[/cyan]' to fetch FIGI data")
            return
...
def search(ctx, isin, mic):
    ...
    try:
        ...
        console.print(strategy_messages.get(search_strategy, f"[yellow]Strategy: {search_strategy}[/yellow]"))
...
@cli.command()
@click.option('--force', is_flag=True, help='Force recreate database (WARNING: destroys existing data)')
...
def init(ctx, force, check_only):
    ...
    try:
        ...
        if init_database(force_recreate=force):
            ...
            console.print("• Download FIRDS data: [yellow]marketdata files download firds --date 2025-09-20[/yellow]")
