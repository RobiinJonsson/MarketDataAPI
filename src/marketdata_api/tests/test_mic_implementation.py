...
def test_api_endpoints():
    ...
    endpoints_to_test = [
        {"name": "List MICs", "url": f"{API_BASE_URL}/mic/?limit=5", "method": "GET"},
        {"name": "Get MIC Statistics", "url": f"{API_BASE_URL}/mic/statistics", "method": "GET"},
        {"name": "List Countries", "url": f"{API_BASE_URL}/mic/countries", "method": "GET"},
        {"name": "Search MICs", "url": f"{API_BASE_URL}/mic/search?q=NASDAQ", "method": "GET"},
        {"name": "Get Enums", "url": f"{API_BASE_URL}/mic/enums", "method": "GET"},
        {
    ...
    for endpoint in endpoints_to_test:
        try:
            ...
            if response.status_code == 200:
                ...
                results.append(
                    {"endpoint": endpoint["name"], "status": "failed", "code": response.status_code}
                )
...
def test_specific_mic_lookup():
    ...
    test_mics = [
        ...
        "XPAR",
        "XFRA",
    ]  # NYSE, NASDAQ, LSE, Euronext Paris, Frankfurt
    ...
    for mic_code in test_mics:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(
                    f"   ✅ {mic_code}: {data.get('market_name', 'Unknown')} ({data.get('iso_country_code', 'Unknown')})"
                )
...
def test_database_queries():
    ...
    try:
        with get_session() as session:
            ...
            active = (
                session.query(MarketIdentificationCode).filter_by(status=MICStatus.ACTIVE).count()
            )
            operating = (
                session.query(MarketIdentificationCode)
                .filter_by(operation_type=MICType.OPRT)
                .count()
            )
