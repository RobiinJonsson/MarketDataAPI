...
from sqlalchemy import Column, String, DateTime, Boolean, Index, Text, Enum as SQLEnum
...
class MarketIdentificationCode(Base):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'lei': self.lei,
            'market_category_code': self.market_category_code.value if self.market_category_code else None,
    ...
    def get_operating_mics(cls, session) -> List['MarketIdentificationCode']:
        ...
        return session.query(cls).filter(
            cls.operation_type == MICType.OPRT,
            cls.status == MICStatus.ACTIVE
        ).all()
    
    @classmethod
    def get_segments_for_operating_mic(cls, session, operating_mic: str) -> List['MarketIdentificationCode']:
    ...
    def get_by_country(cls, session, country_code: str) -> List['MarketIdentificationCode']:
        ...
        return session.query(cls).filter(
            cls.iso_country_code == country_code.upper(),
            cls.status == MICStatus.ACTIVE
        ).all()
