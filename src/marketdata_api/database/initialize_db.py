...
def init_database(force_recreate=False):
    ...
    try:
        ...
        if force_recreate and database_exists():
            if db_type == "sqlserver":
                logger.warning("⚠️ WARNING: About to drop all tables in SQL Server Database - ALL DATA WILL BE LOST")
                user_input = input("Are you sure you want to reset the SQL Server database? (yes/no): ")
