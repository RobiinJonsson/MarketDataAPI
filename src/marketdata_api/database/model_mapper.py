...
def map_to_model(data: Dict[str, Any], instrument_type: str = "equity") -> Dict[str, Any]:
    ...
    mapped = {
        ...
        ),  # Using short name as symbol if no specific symbol
        "currency": data.get("FinInstrmGnlAttrbts_NtnlCcy"),
        "cfi_code": data.get("FinInstrmGnlAttrbts_ClssfctnTp"),
        "commodity_derivative": str(data.get("FinInstrmGnlAttrbts_CmmdtyDerivInd", "false")).lower()
        == "true",  # Proper boolean conversion
    ...
    if instrument_type == "equity":
        mapped.update(
            {
                "admission_approval_date": parse_date(
                    data.get("TradgVnRltdAttrbts_AdmssnApprvlDtByIssr")
                ),
                ...
                "underlying_index_isin": data.get(
                    "DerivInstrmAttrbts_UndrlygInstrm_Sngl_Indx_ISIN"
                ),
                "underlying_single_index_name": data.get(
                ...
                "additional_metal_product": data.get(
        ...
        mapped.update(
            {
                ...
                "floating_rate_term_value": parse_float(
                    data.get("DebtInstrmAttrbts_IntrstRate_Fltg_Term_Val")
                ),
                "floating_rate_basis_points_spread": parse_float(
                    data.get("DebtInstrmAttrbts_IntrstRate_Fltg_BsisPtSprd")
                ),
                "interest_rate_floating_reference_index": data.get(
                    "DebtInstrmAttrbts_IntrstRate_Fltg_RefRate_Indx"
                ),
                "interest_rate_floating_reference_isin": data.get(
...
def map_lei_record(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "lei_record": {
            ...
            or entity.get("legalName", {}).get("name")
            or "Not Available",
        ...
        "addresses": [
            ...
            },
            {
                ...
                "type": "headquarters",
                "address_lines": ", ".join(
                    entity.get("headquartersAddress", {}).get("addressLines", [])
                ),
...
def map_transparency_data(
    data: Dict[str, Any], calculation_type: str, instrument_type: str
...
def _map_non_equity_transparency(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "post_trade_large_in_scale_threshold": parse_float(
            data.get("PstTradLrgInScaleThrshld_Amt")
        ),
        "pre_trade_instrument_size_specific_threshold": parse_float(
            data.get("PreTradInstrmSzSpcfcThrshld_Amt")
        ),
        "post_trade_instrument_size_specific_threshold": parse_float(
            data.get("PstTradInstrmSzSpcfcThrshld_Amt")
        ),
    ...
    base_mapping.update(
        {
            ...
            "pre_trade_large_in_scale_threshold_nb": parse_float(
                data.get("PreTradLrgInScaleThrshld_Nb")
            ),
            "post_trade_large_in_scale_threshold_nb": parse_float(
                data.get("PstTradLrgInScaleThrshld_Nb")
            ),
            "pre_trade_instrument_size_specific_threshold_nb": parse_float(
                data.get("PreTradInstrmSzSpcfcThrshld_Nb")
            ),
            "post_trade_instrument_size_specific_threshold_nb": parse_float(
                data.get("PstTradInstrmSzSpcfcThrshld_Nb")
            ),
