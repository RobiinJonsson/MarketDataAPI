...
class DatabaseBackupManager:
    ...
    def restore_backup(self, backup_path: Path, confirm: bool = False) -> bool:
        ...
        if not confirm:
            response = input(
                f"Are you sure you want to restore from {backup_path.name}? This will overwrite the current database. (yes/no): "
            )
...
def main():
    ...
    cleanup_parser.add_argument(
        "--keep-days", type=int, default=30, help="Keep backups newer than N days"
    )
    cleanup_parser.add_argument(
        "--keep-count", type=int, default=10, help="Keep at least N most recent backups"
    )
    ...
    if args.command == "backup":
        ...
        if not backups:
            ...
            for backup in backups:
                ...
                print(
                    f"{backup['file'].name:<40} {size_mb:>8.1f}MB {backup['created'].strftime('%Y-%m-%d %H:%M'):<20} {backup_type:<15}"
                )
