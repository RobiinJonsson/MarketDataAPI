...
def create_legal_entity_models(api, common_models):
    ...
    legal_entity_base = api.model('LegalEntityBase', {
        'lei': fields.String(required=True, description="Legal Entity Identifier (20 characters)", example="549300DTUYXVMJXZNY12"),
        'name': fields.String(required=True, description="Legal name of the entity"),
        'jurisdiction': fields.String(description="Jurisdiction code (ISO 3166-1)", example="US"),
        'legal_form': fields.String(description="Legal form of the entity"),
        'status': fields.String(description="Current status", enum=["ACTIVE", "INACTIVE", "PENDING"]),
    ...
    entity_address_model = api.model('EntityAddress', {
        'type': fields.String(description="Address type (e.g., HEADQUARTERS, LEGAL)", enum=["HEADQUARTERS", "LEGAL", "ALTERNATIVE"]),
    ...
    entity_registration_model = api.model('EntityRegistration', {
        'status': fields.String(description="Registration status", enum=["ISSUED", "PENDING_VALIDATION", "ISSUED_PENDING_EXCEPTION", "PENDING_TRANSFER", "CANCELLED", "ANNULLED", "RETIRED", "MERGED", "DUPLICATE", "LAPSED"]),
    ...
    legal_entity_create_request = api.model('LegalEntityCreateRequest', {
        'lei': fields.String(required=True, description="Legal Entity Identifier", example="549300DTUYXVMJXZNY12"),
        'name': fields.String(required=True, description="Legal name of the entity"),
        'jurisdiction': fields.String(description="Jurisdiction code", example="US"),
        'legal_form': fields.String(description="Legal form"),
        'status': fields.String(description="Entity status", enum=["ACTIVE", "INACTIVE", "PENDING"])
    })
    
    # Legal entity search request
    legal_entity_search_request = api.model('LegalEntitySearchRequest', {
