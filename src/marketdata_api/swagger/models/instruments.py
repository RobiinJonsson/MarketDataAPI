...
def create_instrument_models(api, common_models):
    ...
    instrument_base = api.model('InstrumentBase', {
        'id': fields.Integer(required=True, description="Internal instrument ID"),
        'isin': fields.String(required=True, description="International Securities Identification Number", example="US0378331005"),
    ...
    instrument_detailed = api.inherit('InstrumentDetailed', instrument_base, {
        'classification': fields.Nested(classification_model, description="CFI classification details"),
        ...
        'trading_venues': fields.List(fields.Raw, description="Trading venues where instrument is listed")
    })
    ...
    venue_info = api.model('VenueInfo', {
    ...
    instrument_venues_response = api.model('InstrumentVenuesResponse', {
        'status': fields.String(required=True, description="Response status", enum=["success"]),
        'data': fields.Nested(api.model('VenuesData', {
            'isin': fields.String(required=True, description="Instrument ISIN"),
            'instrument_type': fields.String(required=True, description="Instrument type"),
            'venue_count': fields.Integer(required=True, description="Number of venues"),
            'venues': fields.List(fields.Nested(venue_info), description="Venue details")
        }))
    })
    
    # Enrichment result model
    enrichment_result = api.model('EnrichmentResult', {
    ...
    instrument_enrichment_response = api.model('InstrumentEnrichmentResponse', {
        'status': fields.String(required=True, description="Response status", enum=["success"]),
        'message': fields.String(required=True, description="Enrichment result message"),
        'id': fields.Integer(required=True, description="Instrument ID"),
        'isin': fields.String(required=True, description="Instrument ISIN"),
        'enrichment_results': fields.Nested(api.model('EnrichmentResults', {
            'figi': fields.Nested(enrichment_result, description="FIGI enrichment result"),
            'lei': fields.Nested(enrichment_result, description="LEI enrichment result")
        }))
    })
    ...
    instrument_create_request = api.model('InstrumentCreateRequest', {
        'isin': fields.String(required=True, description="ISIN code", example="US0378331005"),
        'type': fields.String(required=True, description="Instrument type", example="equity"),
        'cfi_code': fields.String(required=False, description="CFI code for validation (optional)"),
        ...
        'mic_code': fields.String(required=False, description="Market Identification Code (optional)")
    })
