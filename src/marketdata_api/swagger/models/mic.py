...
def create_mic_models(api, common_models):
    ...
    # Basic MIC model
    mic_base = api.model('MICBase', {
        'mic': fields.String(required=True, description="4-character Market Identification Code", example="XNYS"),
        'operating_mic': fields.String(description="Operating MIC for segment MICs", example="XNYS"),
        'oprt_sgmt': fields.String(description="Operating/Segment indicator", enum=["OPRT", "SGMT"]),
        'market_name': fields.String(required=True, description="Official market name", example="NEW YORK STOCK EXCHANGE, INC."),
        'legal_entity_name': fields.String(description="Legal entity operating the market"),
        'lei': fields.String(description="Legal Entity Identifier", example="549300DTUYXVMJXZNY12"),
        'market_category_code': fields.String(description="Market category", enum=["APPA", "REGULATED", "MULTMK", "SMN", "OPEN", "OTHR", "SEFS", "TRFS", "CASP", "IDQS"]),
        'acronym': fields.String(description="Market acronym", example="NYSE"),
        'iso_country_code': fields.String(required=True, description="ISO 3166-1 alpha-2 country code", example="US"),
        ...
        'status': fields.String(description="MIC status", enum=["ACTIVE", "EXPIRED", "DELETED", "MODIFIED"]),
    ...
    mic_detailed = api.inherit('MICDetailed', mic_base, {
        'venue_count': fields.Integer(description="Number of trading venues using this MIC"),
        'segment_mics': fields.List(fields.Nested(mic_base), description="Segment MICs under this operating MIC"),
        'trading_venues': fields.List(fields.Raw, description="Trading venues using this MIC")
    })
    
    # MIC list response
    mic_list_response = api.model('MICListResponse', {
    ...
    # MIC detail response
    mic_detail_response = api.model('MICDetailResponse', {
        'status': fields.String(required=True, description="Response status", enum=["success"]),
        'data': fields.Nested(mic_detailed)
    })
    
    # MIC search request
    mic_search_request = api.model('MICSearchRequest', {
        ...
        'status': fields.String(description="Status filter", enum=["ACTIVE", "EXPIRED", "DELETED", "MODIFIED"]),
        'category': fields.String(description="Category filter", enum=["APPA", "REGULATED", "MULTMK", "SMN", "OPEN", "OTHR", "SEFS", "TRFS", "CASP", "IDQS"]),
    ...
    country_stats = api.model('CountryStats', {
        ...
        'markets': fields.List(fields.Nested(mic_base), description="Sample of markets in country")
    })
    
    # MIC statistics response
    mic_statistics_response = api.model('MICStatisticsResponse', {
        'status': fields.String(required=True, description="Response status", enum=["success"]),
        'data': fields.Nested(api.model('MICStatistics', {
            ...
            'countries_count': fields.Integer(description="Number of countries with MICs"),
            'last_update': fields.DateTime(description="Last data update timestamp"),
            'data_quality': fields.Nested(api.model('DataQuality', {
                'mics_with_lei': fields.Integer(description="MICs with LEI"),
                'mics_with_website': fields.Integer(description="MICs with website"),
                'orphaned_segments': fields.Integer(description="Segment MICs without operating MIC")
            }))
        }))
    })
    
    # Load data request
    mic_load_request = api.model('MICLoadRequest', {
        'source': fields.String(required=True, description="Data source", enum=["local", "remote"]),
        'file_path': fields.String(description="Local file path (for local source)"),
        'force_update': fields.Boolean(description="Force update existing records", default=False)
    })
    
    # Load data response
    mic_load_response = api.model('MICLoadResponse', {
        'status': fields.String(required=True, description="Response status", enum=["success"]),
        'message': fields.String(required=True, description="Operation result message"),
        'data': fields.Nested(api.model('LoadResults', {
            ...
            'file_info': fields.Nested(api.model('FileInfo', {
                'filename': fields.String(description="Source filename"),
                'size': fields.Integer(description="File size in bytes"),
                'last_modified': fields.DateTime(description="File modification time")
            }), description="Source file information")
        }))
    })
    
    # Remote lookup response
    remote_mic_response = api.model('RemoteMICResponse', {
        'status': fields.String(required=True, description="Response status", enum=["success"]),
        'data': fields.Nested(mic_base),
        'meta': fields.Nested(api.model('RemoteMeta', {
    ...
    remote_validation_response = api.model('RemoteValidationResponse', {
    ...
    mic_enums_response = api.model('MICEnumsResponse', {
        'status': fields.String(required=True, description="Response status", enum=["success"]),
        'data': fields.Nested(api.model('MICEnums', {
            'status_values': fields.List(fields.String, description="Valid MIC status values"),
            'category_values': fields.List(fields.String, description="Valid market category values"), 
            'type_values': fields.List(fields.String, description="Valid MIC type values")
        }))
    })
