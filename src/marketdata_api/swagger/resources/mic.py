...
def create_mic_resources(api, models):
        ...
        def get(self):
            ...
            try:
                ...
                with get_session() as session:
                    ...
                    if mic_type:
                        query = query.filter(MarketIdentificationCode.operation_type == mic_type.upper())
                    
                    category = request.args.get('category')
                    if category:
                        query = query.filter(MarketIdentificationCode.market_category_code == category.upper())
    ...
    class MICSegments(Resource):
        @mic_ns.doc(
            ...
            responses={
                ...
                HTTPStatus.BAD_REQUEST: ('MIC is not an operating MIC', common_models['error_model']),
                HTTPStatus.INTERNAL_SERVER_ERROR: ('Internal server error', common_models['error_model'])
            }
        )
        @mic_ns.marshal_with(mic_models['mic_list_response'])
    ...
    class MICSearch(Resource):
        @mic_ns.doc(
            ...
            responses={
                HTTPStatus.OK: ('Success', mic_models['mic_list_response']),
                HTTPStatus.BAD_REQUEST: ('No search criteria provided', common_models['error_model']),
                HTTPStatus.INTERNAL_SERVER_ERROR: ('Internal server error', common_models['error_model'])
            }
        )
        @mic_ns.marshal_with(mic_models['mic_list_response'])
    ...
    class MICLoadData(Resource):
        @mic_ns.doc(
            ...
            responses={
                ...
                HTTPStatus.BAD_REQUEST: ('Invalid request parameters', common_models['error_model']),
                HTTPStatus.INTERNAL_SERVER_ERROR: ('Internal server error', common_models['error_model'])
            }
        )
    ...
    class RemoteMICLookup(Resource):
        @remote_ns.doc(
            ...
            responses={
                ...
                HTTPStatus.NOT_FOUND: ('MIC not found in remote registry', common_models['error_model']),
                HTTPStatus.INTERNAL_SERVER_ERROR: ('Remote service unavailable', common_models['error_model']),
                HTTPStatus.INTERNAL_SERVER_ERROR: ('Internal server error', common_models['error_model'])
            }
        )
        ...
        def get(self, mic_code):
            ...
            try:
                # Call business logic directly instead of route function
                from ...services.mic_data_loader import remote_mic_service, OFFICIAL_MIC_CSV_URL
