...
from requests.exceptions import RequestException, ConnectTimeout, ReadTimeout, HTTPError
...
def retry_with_backoff(max_retries=3, initial_backoff=1, max_backoff=60, backoff_factor=2, 
                      jitter=True, retryable_exceptions=(RequestException,), retryable_status_codes=(429, 500, 502, 503, 504)):
    """
                ...
                try:
                    ...
                    if hasattr(response, 'status_code') and response.status_code in retryable_status_codes:
                        if response.status_code == 429:
                            logger.warning(f"Rate limit exceeded when calling {func.__name__}. Retry {retries+1}/{max_retries}")
                            raise RateLimitError(f"Rate limit exceeded", status_code=response.status_code, response=response)
                        logger.warning(f"Received status code {response.status_code} when calling {func.__name__}. "
                    ...
                    if isinstance(response, dict) and "error" in response:
                        ...
                        if "rate limit" in error_msg.lower():
                            logger.warning(f"Rate limit error in response from {func.__name__}. "
                                          f"Retry {retries+1}/{max_retries}")
                    ...
                    if retries > max_retries:
                        ...
                        if isinstance(e, HTTPError) and hasattr(e, 'response') and e.response.status_code == 429:
                            ...
                            raise ApiError(f"HTTP error after {max_retries} retries", 
                                         status_code=e.response.status_code if hasattr(e, 'response') else None, 
                                         response=e.response if hasattr(e, 'response') else None) from e
                        raise RetryExhaustedError(f"All retry attempts failed for {func.__name__}") from e
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if isinstance(result, dict) and "error" in result:
                logger.error(f"API call failed: {func.__name__} - {result['error']} (took {exec_time:.2f}s)")
            else:
