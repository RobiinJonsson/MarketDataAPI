...
class GLEIFService:
    ...
    def get_lei_data(self, lei_code):
        """
        Get LEI data from GLEIF API.

        Args:
            lei_code: Legal Entity Identifier

        Returns:
            dict: LEI data from GLEIF API
        """
        return fetch_lei_info(lei_code)

    def get_parent_data(self, lei_code):
        """Get parent entity data."""
        return fetch_direct_parent(lei_code)

    def get_children_data(self, lei_code):
        """Get children entities data."""
        return fetch_direct_children(lei_code)

    def sync_relationships(self, session, lei_code, batch_size=100):
        """Sync entity relationships for a given LEI."""
        return sync_entity_relationships(session, lei_code, batch_size)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                return {
                    "error": f"Failed to retrieve direct parent exception information: {str(ex)}"
                }
        else:
            response.raise_for_status()
    except requests.RequestException as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                return {
                    "error": f"Failed to retrieve ultimate parent exception information: {str(ex)}"
                }
        else:
            response.raise_for_status()
    except requests.RequestException as e:
    ...
    if "data" in parent_data and parent_data["data"].get("type") == "lei-records":
        ...
        try:
            ...
            existing_rel = (
                ...
                .filter_by(
                    parent_lei=parent_lei, child_lei=lei_code, relationship_type=relationship_type
                )
                .first()
            )
            ...
            if existing_rel:
                ...
                return {
                    "message": f"Updated existing {relationship_type.lower()} parent relationship"
                }
            else:
                ...
                return {
                    "message": f"Created new {relationship_type.lower()} parent relationship",
                    "parent_lei": parent_lei,
                }
        except Exception as e:
        ...
        existing_exception = (
            session.query(EntityRelationshipException)
            .filter_by(lei=lei_code, exception_type=exception_type)
            .first()
        )
...
def process_children_relationships(
    ...
    current_relationships = (
        session.query(EntityRelationship)
        .filter_by(parent_lei=parent_lei, relationship_type=relationship_type)
        .all()
    )
        ...
        if child.get("type") == "lei-records":
            ...
            try:
                ...
                if child_lei in existing_child_leis:
                    ...
                    existing_rel.last_updated = datetime.now(UTC)
                    results["processed"] += 1
                    results["details"].append({"child_lei": child_lei, "action": "updated"})
                else:
                    # Create new relationship
                    new_rel = EntityRelationship(
                        parent_lei=parent_lei,
                    ...
                    session.add(new_rel)
                    results["processed"] += 1
                    results["details"].append({"child_lei": child_lei, "action": "created"})
                ...
                results["errors"] += 1
                results["details"].append({"child_lei": child_lei, "error": str(e)})
    ...
    if relationships_to_prune:
        for child_lei in relationships_to_prune:
            try:
                ...
                results["pruned"] += 1
                results["details"].append({"child_lei": child_lei, "action": "pruned"})
                ...
                results["details"].append(
                    {"child_lei": child_lei, "action": "prune_failed", "error": str(e)}
                )
...
def prune_parent_relationship(session, lei_code, relationship_type):
    ...
    existing_rel = (
        session.query(EntityRelationship)
        .filter_by(child_lei=lei_code, relationship_type=relationship_type)
        .first()
    )
...
def flatten_address(address: Dict[str, Any], address_type: str, lei: str) -> Dict[str, Any]:
    """Flatten GLEIF address data for database storage"""
    return {
        "lei": lei,
        "type": address_type,
...
def map_lei_record(response: Dict[str, Any]) -> Dict[str, Any]:
    """Maps GLEIF API response to structured data for database storage"""
