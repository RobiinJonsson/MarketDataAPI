...
class SqlServerInstrumentService(InstrumentServiceInterface):
    ...
    def __init__(self):
        ...
        if db_type in ["sqlserver", "azure_sql", "mssql"]:
            ...
            raise ValueError(
                f"SqlServerInstrumentService requires SQL Server database, got: {db_type}"
            )
        ...
        try:
            ...
            if instrument_type == "equity":
                equity_fields = [
                    ...
                    "base_product",
                    "sub_product",
                    "additional_sub_product",
                    "metal_type",
                    "precious_metal",
                    "underlying_single_isin",
                    "basket_isin",
                    "underlying_index_isin",
                    "underlying_single_index_name",
                ]
                ...
                debt_fields = [
                    ...
                    "floating_rate_term_unit",
                    "floating_rate_term_value",
                    "floating_rate_basis_points_spread",
                ]
    ...
    def get_instrument_by_id(self, instrument_id: str) -> Optional[InstrumentInterface]:
        ...
        try:
            result = session.execute(
                text("SELECT * FROM instruments WHERE id = :id"), {"id": instrument_id}
            ).fetchone()
    ...
    def get_instrument_by_isin(self, isin: str) -> Optional[InstrumentInterface]:
        ...
        try:
            result = session.execute(
                text("SELECT * FROM instruments WHERE isin = :isin"), {"isin": isin}
            ).fetchone()
    ...
    def search_instruments(self, query: str, limit: int = ServiceDefaults.DEFAULT_SEARCH_LIMIT) -> List[InstrumentInterface]:
        ...
        try:
            ...
            params = {"query": f"%{query}%", "exact_query": query, "limit": limit}
