...
def dev_docs():
    ...
    return send_from_directory(
        os.path.join(current_app.root_path, '..', 'docs', 'development'),
        'README.md'
    )
...
def regenerate_docs():
    ...
    try:
        ...
        if not current_app.config.get('TESTING') and not current_app.config.get('DEBUG'):
            return jsonify({"error": "Documentation regeneration only available in development mode"}), 403
        ...
        return jsonify({
            ...
            "message": "Documentation regeneration completed" if result.returncode == 0 else "Documentation regeneration failed",
