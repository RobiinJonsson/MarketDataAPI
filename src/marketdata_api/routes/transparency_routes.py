...
def list_transparency_calculations():
    ...
    try:
        ...
        logger.info(
            f"GET /transparency with enhanced params: file_type={file_type}, category={instrument_category}, is_equity={is_equity}, is_debt={is_debt}, isin={isin}"
        )
...
def get_transparency_calculation(transparency_id):
    ...
    try:
        ...
        try:
            ...
            if not calculation:
                return (
                    jsonify({ResponseFields.ERROR: "Transparency calculation not found"}),
            ...
            if not result:
                return (
                    jsonify({ResponseFields.ERROR: "Error formatting transparency calculation"}),
                    HTTPStatus.INTERNAL_SERVER_ERROR,
                )
...
def create_transparency_calculation():
    ...
    try:
        ...
        if "TechRcrdId" in data:
            ...
            calculations = service.create_transparency(isin=isin, instrument_type=instrument_type)

        if not calculations:
            return (
                jsonify({ResponseFields.ERROR: "Failed to create transparency calculation"}),
                HTTPStatus.BAD_REQUEST,
            )
        ...
        if not result:
            return (
                jsonify(
                    {ResponseFields.ERROR: "Error formatting created transparency calculation"}
                ),
                HTTPStatus.INTERNAL_SERVER_ERROR,
            )

        logger.info(
            f"Successfully created {len(calculations)} transparency calculation(s) for {isin}"
        )
...
def process_fitrs_file():
    ...
    try:
        ...
        if not source_filename or not file_data:
            return (
                jsonify({ResponseFields.ERROR: "Both source_filename and file_data are required"}),
                HTTPStatus.BAD_REQUEST,
            )
        ...
        logger.info(
            f"Successfully processed {len(formatted_results)} transparency calculations from {source_filename}"
        )
    ...
    try:
        ...
        try:
            ...
            file_type_stats = (
                ...
                .group_by(TransparencyCalculation.file_type)
                .all()
            )
            ...
            transaction_data_stats = session.query(
                ...
                func.count(TransparencyCalculation.total_transactions_executed).label(
                    "with_transactions"
                ),
            ...
            equity_stats = session.query(
                func.sum(
                    func.case([(TransparencyCalculation.file_type.like("FULECR_%"), 1)], else_=0)
                ).label("equity_count"),
                func.sum(
                    func.case([(TransparencyCalculation.file_type.like("FULNCR_%"), 1)], else_=0)
                ).label("non_equity_count"),
            ).first()
            ...
            transaction_fill_rate = (
                (transaction_data_stats.with_transactions / total_records * 100)
                if total_records > 0
                else 0
            )
            volume_fill_rate = (
                (transaction_data_stats.with_volume / total_records * 100)
                if total_records > 0
                else 0
            )
            ...
            result = {
                ...
                "data_availability": {
                    "total_records": total_records,
                    "records_with_transactions": (
                        transaction_data_stats.with_transactions if transaction_data_stats else 0
                    ),
                    "records_with_volume": (
                        transaction_data_stats.with_volume if transaction_data_stats else 0
                    ),
            ...
            return (
                jsonify(
                    {ResponseFields.STATUS: f"{HTTPStatus.OK} OK", ResponseFields.DATA: result}
                ),
                HTTPStatus.OK,
            )
