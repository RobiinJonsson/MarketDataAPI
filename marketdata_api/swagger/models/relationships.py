...
def create_relationship_models(api, common_models):
    ...
    relationship_base = api.model('RelationshipBase', {
        'relationship_type': fields.String(required=True, description="Type of relationship", enum=["DIRECT", "ULTIMATE"]),
        'relationship_status': fields.String(required=True, description="Status of the relationship", enum=["ACTIVE", "INACTIVE"]),
    ...
    entity_summary = api.model('EntitySummary', {
    ...
    relationship_list_response = api.model('RelationshipListResponse', {
        ...
        'data': fields.Nested(api.model('RelationshipData', {
            'entity': fields.Nested(entity_summary, required=True, description="The queried entity"),
            'relationships': fields.List(fields.Nested(relationship_base), required=True, description="List of relationships")
        })),
        'meta': fields.Nested(common_models['pagination_meta'])
    })
    
    # Relationship creation request
    relationship_create_request = api.model('RelationshipCreateRequest', {
        'parent_lei': fields.String(required=True, description="LEI of the parent entity"),
        'child_lei': fields.String(required=True, description="LEI of the child entity"),
        'relationship_type': fields.String(required=True, description="Type of relationship", enum=["DIRECT", "ULTIMATE"]),
        'relationship_status': fields.String(description="Status of the relationship", enum=["ACTIVE", "INACTIVE"], default="ACTIVE"),
        'percentage_ownership': fields.Float(description="Percentage of ownership (0-100)"),
        'relationship_period_start': fields.DateTime(description="Start date of the relationship"),
        'relationship_period_end': fields.DateTime(description="End date of the relationship")
    })
    ...
    relationship_hierarchy_response = api.model('RelationshipHierarchyResponse', {
        'status': fields.String(required=True, description="Response status", enum=["success"]),
        'data': fields.Nested(api.model('HierarchyData', {
            'root_entity': fields.Nested(entity_summary, description="Root entity"),
            'hierarchy': fields.Raw(description="Hierarchical structure of relationships"),
