...
class MICDataLoader:
    ...
    def load_from_csv(self, csv_source: Union[str, io.StringIO], data_version: Optional[str] = None) -> Tuple[int, int, List[str]]:
        """
        ...
        logger.info(f"Loading MIC data from {'remote source' if isinstance(csv_source, io.StringIO) else csv_source}")
            ...
            try:
                ...
                for row_num, row in enumerate(reader, start=2):  # Start at 2 for header
                    try:
                        ...
                        if mic_data:
                            existing_mic = self.session.query(MarketIdentificationCode).filter_by(
                                mic=mic_data['mic']
                            ).first()
                ...
                # Commit changes
                self.session.commit()
                logger.info(f"MIC data loaded: {created_count} created, {updated_count} updated, {len(errors)} errors")
    ...
    def load_from_remote_url(self, url: str = OFFICIAL_MIC_CSV_URL, data_version: Optional[str] = None) -> Tuple[int, int, List[str]]:
        """
    ...
    def _transform_csv_row(self, row: Dict[str, str], data_version: str) -> Optional[Dict[str, Any]]:
    ...
    def get_load_statistics(self) -> Dict[str, Any]:
        ...
        active_mics = self.session.query(MarketIdentificationCode).filter_by(status=MICStatus.ACTIVE).count()
        operating_mics = self.session.query(MarketIdentificationCode).filter_by(
            operation_type=MICType.OPRT, status=MICStatus.ACTIVE
        ).count()
        segment_mics = self.session.query(MarketIdentificationCode).filter_by(
            operation_type=MICType.SGMT, status=MICStatus.ACTIVE
        ).count()
    ...
    def validate_mic_relationships(self) -> List[str]:
        ...
        orphaned_segments = self.session.query(MarketIdentificationCode).filter(
            MarketIdentificationCode.operation_type == MICType.SGMT,
            ~self.session.query(MarketIdentificationCode).filter(
        ...
        for segment in orphaned_segments:
            issues.append(f"Segment MIC {segment.mic} references non-existent operating MIC {segment.operating_mic}")
        
        return issues


def load_mic_data_from_csv(session: Session, csv_source: Union[str, None] = None, data_version: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    def search_mics(self, query: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for mic_code, mic_info in data.items():
                if (query in mic_code.lower() or 
                    query in mic_info.get('market_name', '').lower() or
                    query in mic_info.get('legal_entity_name', '').lower() or
                    query in mic_info.get('acronym', '').lower()):
    ...
    def validate_mic(self, mic_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if mic_info is None:
                return {
                    'valid': False,
                    'error': f'MIC code {mic_code} not found in ISO registry'
                }
            ...
            return {
                'valid': False,
                'error': f'Failed to validate MIC {mic_code}: {str(e)}'
            }
