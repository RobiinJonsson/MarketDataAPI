...
import uuid
from sqlalchemy import Column, String, DateTime, Float, Date, ForeignKey, Boolean, Integer, Index, JSON
    ...
    def get_criteria_pairs(self):
        ...
        for i in range(1, 8):  # Up to 7 criteria pairs in futures
            ...
            if name_key in self.raw_data and value_key in self.raw_data:
                criteria.append({
                    "name": self.raw_data[name_key],
                    "value": self.raw_data[value_key]
                })
        return criteria
...
class TransparencyThreshold(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    transparency_id = Column(String, ForeignKey('transparency_calculations.id', ondelete='CASCADE'), nullable=False)
