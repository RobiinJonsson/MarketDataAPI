    ...
    def decode_attributes(cls, cfi_code: str) -> Dict[str, Any]:
        ...
        if category == 'E':
            if group == 'S':  # Common shares
                return {
                    "voting_rights": cls.EQUITY_VOTING_RIGHTS.get(attrs[0], f"Unknown ({attrs[0]})"),
                    "ownership_restrictions": cls.EQUITY_OWNERSHIP_RESTRICTIONS.get(attrs[1], f"Unknown ({attrs[1]})"),
                    "payment_status": cls.EQUITY_PAYMENT_STATUS.get(attrs[2], f"Unknown ({attrs[2]})"),
                ...
                return {
                    "instrument_dependency": cls.DEPOSITORY_DEPENDENCY.get(attrs[0], f"Unknown ({attrs[0]})"),
                    "redemption_conversion": cls.PREFERRED_REDEMPTION.get(attrs[1], f"Unknown ({attrs[1]})"),
                    "income": cls.PREFERRED_INCOME.get(attrs[2], f"Unknown ({attrs[2]})"),
                    "form": cls.EQUITY_FORM.get(attrs[3], f"Unknown ({attrs[3]})")
                }
                ...
                return {
                    ...
                    "distribution": "Dividend payments" if attrs[1] == 'D' else "No payments" if attrs[1] == 'Y' else f"Unknown ({attrs[1]})",
                    "repayment": "Cash repayment" if attrs[2] == 'F' else "Physical repayment" if attrs[2] == 'V' else "Elect at settlement" if attrs[2] == 'E' else f"Unknown ({attrs[2]})",
                    "underlying_assets": "Baskets" if attrs[3] == 'B' else "Equities" if attrs[3] == 'S' else "Debt instruments" if attrs[3] == 'D' else f"Unknown ({attrs[3]})"
                }
        ...
        # Collective Investment Vehicles
            ...
            if group == 'I':  # Standard funds
                return {
                    ...
                    "distribution_policy": cls.CIV_DISTRIBUTION_POLICY.get(attrs[1], f"Unknown ({attrs[1]})"),
                ...
                return {
                    "investment_strategy": cls.HEDGE_STRATEGY.get(attrs[0], f"Unknown ({attrs[0]})"),
                ...
                return {
                    "closed_open_end": cls.CIV_CLOSED_OPEN.get(attrs[0], f"Unknown ({attrs[0]})"),
                    "distribution_policy": cls.CIV_DISTRIBUTION_POLICY.get(attrs[1], f"Unknown ({attrs[1]})"),
                    "assets": cls.CIV_ASSETS.get(attrs[2], f"Unknown ({attrs[2]})") if group != 'B' else "Real estate",
                    "security_type": cls.CIV_SECURITY_TYPE.get(attrs[3], f"Unknown ({attrs[3]})")
                }
