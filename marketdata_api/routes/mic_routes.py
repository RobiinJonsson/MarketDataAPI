...
def get_mic_segments(mic_code: str):
    ...
    with get_session() as session:
        ...
        operating_mic = session.query(MarketIdentificationCode).filter_by(
            mic=mic_code.upper(),
            operation_type=MICType.OPRT
        ).first()
...
def load_mic_data():
    ...
    if source == 'local' and not csv_path:
        return jsonify({
            'error': 'csv_path is required when source is "local"'
        }), 400
    
    if source not in ['remote', 'local']:
        return jsonify({
            'error': 'source must be either "remote" or "local"'
        }), 400
    ...
    with get_session() as session:
        try:
            if source == 'remote':
                ...
                result = load_mic_data_from_csv(session, csv_source=csv_path, data_version=data_version)
...
def remote_lookup_mic(mic_code: str):
    ...
    try:
        ...
        if mic_info is None:
            return jsonify({
                'error': f'MIC code {mic_code.upper()} not found in ISO registry'
            }), 404
        ...
        return jsonify({
            'error': f'Failed to lookup MIC from remote source: {str(e)}'
        }), 500
...
def remote_search_mics():
    ...
    try:
        ...
        return jsonify({
            'error': f'Failed to search MICs from remote source: {str(e)}'
        }), 500
...
def remote_get_country_mics(country_code: str):
    ...
    try:
        ...
        return jsonify({
            'error': f'Failed to get country MICs from remote source: {str(e)}'
        }), 500
...
def clear_remote_cache():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Remote MIC cache cleared successfully'
        })
    except Exception as e:
        return jsonify({
            'success': False,
