    ...
    try:
        ...
        for test_instrument in TEST_INSTRUMENTS:
            ...
            try:
                ...
                if calculations:
                    logger.info(f"✅ SUCCESS: Found/created {len(calculations)} transparency calculation(s)")
                    for i, calc in enumerate(calculations, 1):
                        ...
                        logger.info(f"     - Total Transactions: {calc.total_transactions_executed}")
                        ...
                        if calculation_type == 'EQUITY':
                            if hasattr(calc, 'equity_transparency') and calc.equity_transparency:
                                ...
                                logger.info(f"       * Methodology: {calc.equity_transparency.methodology}")
                                logger.info(f"       * Average Daily Turnover: {calc.equity_transparency.average_daily_turnover}")
                            else:
                            ...
                            if hasattr(calc, 'debt_transparency') and calc.debt_transparency:
                                ...
                                logger.info(f"       * Bond Type: {calc.debt_transparency.bond_type}")
                                logger.info(f"       * Is Liquid: {calc.debt_transparency.is_liquid}")
                                details_found = True
                            elif hasattr(calc, 'futures_transparency') and calc.futures_transparency:
                                logger.info(f"     - Futures Details: Available")
                                logger.info(f"       * Underlying ISIN: {calc.futures_transparency.underlying_isin}")
                                logger.info(f"       * Is Stock Dividend Future: {calc.futures_transparency.is_stock_dividend_future}")
                                details_found = True
                            elif hasattr(calc, 'non_equity_transparency') and calc.non_equity_transparency:
                                logger.info(f"     - Non-Equity Details: Available")
                                logger.info(f"       * Description: {calc.non_equity_transparency.description}")
                                logger.info(f"       * Criterion Name: {calc.non_equity_transparency.criterion_name}")
                                details_found = True
                    ...
                    if calculations_retry:
                        logger.info(f"✅ SUCCESS (retry): Found/created {len(calculations_retry)} transparency calculation(s)")
                        results.append({
                            'isin': isin,
                            'type': instrument_type,
                            'calculation_type': calculation_type,
        ...
        if successful:
            ...
            for result in successful:
                logger.info(f"  - {result['isin']} ({result['type']}): {result['count']} calculation(s)")
...
def test_database_content():
    ...
    try:
        ...
        try:
            ...
            equity_count = session.query(TransparencyCalculation).filter(
                TransparencyCalculation.calculation_type == 'EQUITY'
            ).count()
            non_equity_count = session.query(TransparencyCalculation).filter(
                TransparencyCalculation.calculation_type == 'NON_EQUITY'
            ).count()
            ...
            for test_instrument in TEST_INSTRUMENTS:
                ...
                if existing:
                    ...
                    for calc in existing:
                        logger.info(f"     - ID: {calc.id}, Type: {calc.calculation_type}, Tech ID: {calc.tech_record_id}")
                else:
